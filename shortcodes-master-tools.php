 ...
 */
function sm_hex_shift( $supplied_hex, $shift_method, $percentage = 50 ) {
...
function sm_hex2rgb( $colour, $delimiter = '-' ) {
	if ( $colour[0] == '#' ) {
 ...
 */
function sm_apply_formatting() {
	// Enable shortcodes in text widgets
	add_filter( 'widget_text', 'do_shortcode' );
	// Enable shortcodes in category descriptions
	add_filter( 'category_description', 'do_shortcode' );
	// Enable custom formatting
	if ( get_option( 'sm_option_custom-formatting' ) === 'on' ) {
		// Apply custom formatter function
		add_filter( 'the_content', 'sm_clean_shortcodes' );
	}
}

add_action( 'init', 'sm_apply_formatting' );
 ...
 */
function sm_clean_shortcodes( $content ) {
	$p = sm_cmpt();
	$array = array (
		'<p>[' => '[',
		']</p>' => ']',
 ...
 */
function sm_do_shortcode( $content, $pre ) {
 ...
 */
function sm_compatibility_mode_prefix() {
	return get_option( 'sm_option_prefix' );
}

/**
 * Shortcut for sm_compatibility_mode_prefix()
 */
function sm_cmpt() {
	return sm_compatibility_mode_prefix();
}
 ...
 * @return bool
 */
function sm_addon_active( $addons ) {
	// Prepare add-ons paths
	$paths = array(
		'maker' => 'shortcodes-master-maker.php'
	);
...
function sm_skins_link() {
	if ( sm_addon_active( 'skins' ) ) return sprintf( '<br><strong>%s</strong><br><strong>%s</strong>', __( 'Additional skins successfully installed', 'sm' ), __( 'Open dropdown to choose one of new styles', 'sm' ) );
	else return sprintf( '<br><a href="https://lizatom.com/club/" target="_blank">%s &rarr;</a>', __( 'Get more styles', 'sm' ) );
}
 ...
 */
function sm_image_resize( $url, $width = NULL, $height = NULL, $crop = true, $retina = false ) {
	global $wp_version;
 ...
 */
function sm_delete_resized_images( $post_id ) {
...
add_action( 'delete_attachment', 'sm_delete_resized_images' );

class Sm_Tools {
	function __construct() {
		add_action( 'wp_ajax_sm_example_preview', array( __CLASS__, 'example' ) );
		add_action( 'sm/update',                  array( __CLASS__, 'reset_examples' ) );
		add_action( 'sm/activation',              array( __CLASS__, 'reset_examples' ) );
	...
	public static function get_slides( $args ) {
		...
		if ( $args['gallery'] !== null || ( $args['source'] === 'none' && get_option( 'sm_option_galleries-432' ) ) ) return self::get_slides_432( $args );
		...
		if ( is_array( $query->posts ) ) foreach ( $query->posts as $post ) {
				...
				elseif ( $args['link'] === 'custom' ) $slide['link'] = get_post_meta( $post->ID, 'sm_slide_link', true );
	...
	public static function get_slides_432( $args ) {
		...
		$galleries = get_option( 'sm_option_galleries-432' );
		// No galleries found
	...
	public static function example() {
		...
		// Check for cache
		$output = get_transient( 'sm/examples/render/' . sanitize_key( $_REQUEST['id'] ) );
		...
		// Cache not found
			...
			// Check for code
			if ( !$code ) die( '<p class="sm-examples-error">' . __( 'Example code does not found, please check it later', 'sm' ) . '</p>' );
			// Clean-up the code
			$code = str_replace( array( "\t", '%sm_' ), array( '  ', sm_cmpt() ), $code );
			// Split code
			$chunks = explode( '-----', $code );
			// Show snippets
			do_action( 'sm/examples/preview/before' );
			foreach ( $chunks as $chunk ) {
				...
				echo '<div class="sm-examples-code"><span class="sm-examples-get-code button"><i class="fa fa-code"></i>&nbsp;&nbsp;' . __( 'Get the code', 'sm' ) . '</span><textarea rows="' . $rows . '">' . esc_textarea( $chunk ) . '</textarea></div>';
			}
			do_action( 'sm/examples/preview/after' );
			$output = ob_get_contents();
			ob_end_clean();
			set_transient( 'sm/examples/render/' . sanitize_key( $_REQUEST['id'] ), $output );
			echo $output;
		}
		die();
	}
	...
	public static function reset_examples() {
		// foreach ( (array) Sm_Data::examples() as $example ) foreach ( (array) $example['items'] as $item ) delete_transient( 'sm/examples/render/' . $item['id'] );
	}
	...
	public static function icons() {
		$icons = array();
		if ( is_callable( array( 'Sm_Data', 'icons' ) ) ) foreach ( (array) Sm_Data::icons() as $icon ) {
	...
	public static function images() {
		$images = array();
		if ( is_callable( array( 'Sm_Data', 'images' ) ) ) foreach ( (array) Sm_Data::images() as $image ) {
				$images[] = $image;
			}
	...
	public static function slide_link_input( $form_fields, $post ) {
		$form_fields['sm_slide_link'] = array(
			'label' => __( 'Slide link', 'sm' ),
			'input' => 'text',
			'value' => get_post_meta( $post->ID, 'sm_slide_link', true ),
			'helps' => sprintf( '<strong>%s</strong><br>%s', __( 'Shortcodes Master', 'sm' ), __( 'Use this field to add custom links to slides used with Slider, Carousel and Custom Gallery shortcodes', 'sm' ) )
		);
		return $form_fields;
	}

	public static function slide_link_save( $post, $attachment ) {
		if ( isset( $attachment['sm_slide_link'] ) )
			update_post_meta( $post['ID'], 'sm_slide_link', $attachment['sm_slide_link'] );
		return $post;
	}

	public static function error( $prefix = false, $message = false ) {
		if ( !$prefix && !$message ) return '';
		$return = array( '<div class="sm-error" style="padding:10px;border:1px solid #f03;color:#903;background:#fde">' );
...
/**
 * Shortcut for Sm_Tools::decode_shortcode()
 */
function sm_scattr( $value ) {
	return Sm_Tools::do_attr( $value );
}

/**
 * Shortcut for Sm_Tools::get_icon()
 */
function sm_get_icon( $args ) {
	return Sm_Tools::get_icon( $args );
}

/**
 * Shortcut for Sm_Tools::get_image()
 */
function sm_get_image( $args ) {
	return Sm_Tools::get_image( $args );
}
